...
public class OpenAccount {
	...
	public void openAccount(
		...
		int totalAcctHolders = firstNames.length;
		...
		wait.until(ExpectedConditions.visibilityOf(driver.findElements(By.xpath("//input[@name='custCategory']")).get(Integer.parseInt(prodType))));
		...
		// your info
		driver.findElements(By.xpath("//input[@name='ownerType']")).get(1).click();
		...
		for(int i=0;i<totalAcctHolders;i++){
			/*
			driver.findElements(By.xpath("//input[@id='enterInfo']")).get(i).click();
			if(i!=0){// joint account
				//click on continue
			}
			*/
